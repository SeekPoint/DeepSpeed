...
gd.debuginfo(prj="dsST", info=f".......ds setpy.py import ends....")

# Fetch rocm state.
is_rocm_pytorch = OpBuilder.is_rocm_pytorch()
rocm_version = OpBuilder.installed_rocm_version()

gd.debuginfo(prj="dsST", info=f"is_rocm_pytorch is {is_rocm_pytorch} +++ rocm_version is: {rocm_version}")
...
if torch_available and torch.cuda.is_available():
    ...
    if is_rocm_pytorch:
        ...
        gd.debuginfo(prj="dsST", info=f"rocm_major is {rocm_major} +++ rocm_minor is: {rocm_minor}")
        # XXX cupy support for rocm 5 is not available yet.
        if rocm_major <= 4:
            cupy = f"cupy-rocm-{rocm_major}-{rocm_minor}"
    else:
        cuda_major_ver, cuda_minor_ver = installed_cuda_version()
        gd.debuginfo(prj="dsST", info=f"cuda_major_ver is {cuda_major_ver} +++ cuda_minor_ver is: {cuda_minor_ver}")
    ...
    gd.debuginfo(prj="dsST", info=f'cupy is: {cupy}')

    if cupy:
        gd.debuginfo(prj="dsST")
...
for extra in extras_require.items():
    gd.debuginfo(prj="dsST", info=f"extra is: {extra}")
    for req in extra[1]:
        gd.debuginfo(prj="dsST", info=f"req is: {req}")
        all_extras.add(req)

extras_require['all'] = list(all_extras)

gd.debuginfo(prj="dsST", info=f"extras_require['all']: {extras_require['all']}")
...
if torch_available:
    ...
    gd.debuginfo(prj="dsST", info=f"cmdclass['build_ext']: {cmdclass['build_ext']}")

if torch_available:
    TORCH_MAJOR = torch.__version__.split('.')[0]
    TORCH_MINOR = torch.__version__.split('.')[1]
    gd.debuginfo(prj="dsST", info=f"TORCH_MAJOR is {TORCH_MAJOR}, TORCH_MINOR is{TORCH_MINOR}")
else:
    gd.debuginfo(prj="dsST", info=f"TORCH is NOT available")
    TORCH_MAJOR = "0"
    TORCH_MINOR = "0"
...
if torch_available and not torch.cuda.is_available():
    ...
    if os.environ.get("TORCH_CUDA_ARCH_LIST", None) is None:
        ...
        gd.debuginfo(prj="dsST", info=f'os.environ["TORCH_CUDA_ARCH_LIST"]: {os.environ["TORCH_CUDA_ARCH_LIST"]}')
...
gd.debuginfo(prj="dsST", info=f"BUILD_OP_PLATFORM is: {BUILD_OP_PLATFORM}")

BUILD_OP_DEFAULT = int(os.environ.get('DS_BUILD_OPS', BUILD_OP_PLATFORM))
gd.debuginfo(prj="dsST", info=f"DS_BUILD_OPS is: {BUILD_OP_DEFAULT}")
...
gd.debuginfo(prj="dsST", info=f"compatible_ops is: {compatible_ops}")
gd.debuginfo(prj="dsST", info=f"install_ops is: {install_ops}")
gd.debuginfo(prj="dsST", info=f"ALL_OPS.items() is: {ALL_OPS.items()}")

for op_name, builder in ALL_OPS.items():
    gd.debuginfo(prj="dsST", info=f"op_name is: {op_name} +++ builder is: {builder}")
...
gd.debuginfo(prj="dsST", info=f"ext_modules is: {ext_modules}")
gd.debuginfo(prj="dsST", info=f'Install Ops = {install_ops}')
...
if command_exists('git') and 'DS_BUILD_STRING' not in os.environ:
    try:
        ...
        gd.debuginfo(prj="dsST", info=f"result --A is: {result}")
        
        git_hash = result.decode('utf-8').strip()
        gd.debuginfo(prj="dsST", info=f"git_hash is: {git_hash}")
        
        result = subprocess.check_output(git_branch_cmd, shell=True)
        gd.debuginfo(prj="dsST", info=f"result --B is: {result}")
        
        git_branch = result.decode('utf-8').strip()
        gd.debuginfo(prj="dsST", info=f"git_branch is: {git_branch}")
...
gd.debuginfo(prj="dsST", info=f"version_str --A is: {version_str}")
...
if 'DS_BUILD_STRING' in os.environ:
    ...
    gd.debuginfo(prj="dsST", info=f"version_str --B is: {version_str}")
elif os.path.isfile('build.txt'):
    # build.txt exists, probably installing from distribution.
    with open('build.txt', 'r') as fd:
        version_str += fd.read().strip()
    gd.debuginfo(prj="dsST", info=f"version_str --C is: {version_str}")
else:
    # None of the above, probably installing from source.
    version_str += f'+{git_hash}'
    gd.debuginfo(prj="dsST", info=f"version_str --D is: {version_str}")
...
gd.debuginfo(prj="dsST", info=f"torch_info={torch_info}")

gd.debuginfo(prj="dsST", info=f"version={version_str}, git_hash={git_hash}, git_branch={git_branch}")
...
gd.debuginfo(prj="dsST", info=f"install_requires={install_requires}")
gd.debuginfo(prj="dsST", info=f"compatible_ops={compatible_ops}")
gd.debuginfo(prj="dsST", info=f"ext_modules={ext_modules}")
...
gd.debuginfo(prj="dsST", info=f"extras_require: + str(extras_require)")

start_time = time.time()
gd.debuginfo(prj="dsST", info=f".................start call setup...........\n\n\n")
...
gd.debuginfo(prj="dsST", info=f".................call setup ends...........\n\n\n")
